...
pub const Model = struct {
    ...
    pub fn embedInto(self: *const Model, scratch: std.mem.Allocator, text: []const u8, out: []f32) !void {
        ...
        for (ids.items) |id| {
            ...
            const row = self.embeddings[@as(usize, id) * self.dim ..][0..self.dim];
            for (out, row) |*o, r| o.* += r;
            used += 1;
        }
...
test "matches reference embeddings for potion-base-8M" {
    const model_dir = "models/potion-base-8M";
    ...
    for (golden.array.items) |case| {
        ...
        if (max_diff > 1e-5) {
            std.debug.print("parity failure on \"{s}\": max diff {d}\n", .{ text, max_diff });
